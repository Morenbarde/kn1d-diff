...
def sigmav_el_h_p( T, E, use_Bspline = 0):
    ...
    LTH = np.log(_T)
    if use_Bspline:
        # JSigmaV_EL_H_P common block - will change later 
        EKnot_EL_H_P=None
        TKnot_EL_H_P=None
        order_EL_H_P=None
        LogSigmaV_EL_H_P_BSCoef=None
        if LogSigmaV_EL_H_P_BSCoef is None:
            # haven't talked about how to restore yet
            pass 
        LEP = np.maximum(LEP, min(EKnot_EL_H_P))
        LEP = np.minimum(LEP, max(EKnot_EL_H_P))
        LTH = np.maximum(LTH, min(TKnot_EL_H_P))
        LTH = np.minimum(LTH, max(TKnot_EL_H_P))
        result = np.exp( ) # still haven't figured out what to do with BS2DR
    else:
        # SIGMAV_EL_H_P_DATA common block
        Ln_E_Particle=None
        Ln_T_Target=None
        SigmaV=None
        nEP=None
        ...
        LTH = np.maximum(LTH, min(Ln_T_Target))
        LTH = np.minimum(LTH, max(Ln_T_Target))
