...
class KineticH():
    ...
    def _compile_results(self, fH, nH, fSH, gamma_wall, alpha_c, Beta_CX_sum, collision_freqs, m_sums):
        ...
        #	Albedo
        gammax_plus = self.vth*torch.sum(dvr_vol*(fH[:,self.vx_pos,0] @ (vx[self.vx_pos]*dvx[self.vx_pos]))) 
        gammax_minus = self.vth*torch.sum(dvr_vol*(fH[:,self.vx_neg,0] @ (vx[self.vx_neg]*dvx[self.vx_neg])))
        AlbedoH = torch.tensor(0.0, dtype=self.dtype, device=self.device)
    ...
    def _init_grid(self):
        ...
        self.Internal.vr2vx2 = torch.tile((self.mesh.vr[:,None]**2 + self.mesh.vx[None,:]**2)[:,:,None], (1, 1, self.nx))
    ...
    def _init_protons(self):
        ...
        self.Internal.Ti_mu = torch.broadcast_to(self.mesh.Ti[None, None, :]/self.mu, (self.nvr, self.nvx, self.nx))
    ...
    def _init_sigv(self):
        ...
        if self.ion_rate_option == "jh": # NOTE Not Torch Safe Yet
    ...
    def _init_sig_cx(self):
        ...
        _Sig = self.Internal.v_v*sigma_cx_h0(sig_in)
    ...
    def _init_sig_h_h(self):
        ...
        sig_out = sigma_el_h_h(sig_in, vis=True)
    ...
    def _init_sig_h_p(self):
        ...
        sig_in = self.Internal.v_v2*(0.5*CONST.H_MASS*(self.vth**2) / CONST.Q)
        _Sig = self.Internal.v_v*sigma_el_p_h(sig_in)
        _Sig = torch_reshape_fortran(_Sig, (self.nvr*self.nvx*self.nvr*self.nvx,self.ntheta))
    ...
    def _compute_dynamic_internals(self, fH, fH2, nHP, THP, fSH):
        ...
        # Reset H2 Moments
        self.H2_Moments.nH2 = torch.zeros(self.nx, dtype=self.dtype, device=self.device)
        self.H2_Moments.VxH2 = torch.zeros(self.nx, dtype=self.dtype, device=self.device)
        self.H2_Moments.TH2 = torch.full((self.nx,), 1.0, dtype=self.dtype, device=self.device)

        # if New_H_Seed:
        self.Internal.MH_H_sum = torch.zeros((self.nvr,self.nvx,self.nx), dtype=self.dtype, device=self.device)
    ...
    def _compute_ni(self, nHP):
        ...
        self.Internal.ni = torch.maximum(self.Internal.ni, 0.01*self.mesh.ne)
